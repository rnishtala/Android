...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Integer> feedIds = MainActivity.sRawTextFeedIds;
		Integer[] feedIdArray = feedIds.toArray(new Integer[feedIds.size()]);
		
		// TODO: Start the DownloaderTask
		mdownloaderTask.execute(feedIdArray);
	...
	// the hosting Activity using the DownloadFinishedListener interface.
	 ...
	 public class DownloaderTask extends AsyncTask<Integer,Integer,String[]> {
		 ...
		 protected String[] doInBackground(Integer... feedIds){
			 String[] feeds = downloadTweets(feedIds);
			 if(feeds != null){
				 return feeds;
			 }
			 return null;
					...
					new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (getResultCode() != MainActivity.IS_ALIVE) {
								...
								PendingIntent mContentIntent = PendingIntent.getActivity(context,1,intent,PendingIntent.FLAG_UPDATE_CURRENT);
